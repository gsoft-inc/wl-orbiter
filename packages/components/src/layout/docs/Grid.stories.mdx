...
<Preview scope={{ ShareGateTheme }}>
    <Story name="repeat function">
        <Grid
            ...
            gap={2}
        >
            {ShareGateTheme.colors.common.purple.map(x => <Div backgroundColor={x} key={x} />)}
            {ShareGateTheme.colors.common.alert.map(x => <Div backgroundColor={x} key={x} />)}
            {ShareGateTheme.colors.common.green.map(x => <Div backgroundColor={x} key={x} />)}
        </Grid>
    </Story>
</Preview>
